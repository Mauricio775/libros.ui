...
import { inventoryService, bookService } from '../services';
...
const InventoryList = () => {
    ...
    const loadData = async () => {
        try {
            ...
            // Cargamos los Inventarios (ya incluyen catalog_type_description)
            const catalogsData = await inventoryService.getAll();
            setInventories(catalogsData);

            // Cargamos los tipos de libros para el formulario
            try {
                const bookData = await bookService.getAll();
                setBooks(bookData);
            } catch (typeError) {
                console.warn('Error al cargar  libros:', typeError);
                setBooks([]);
            }
    ...
    const handleDelete = async (item) => {
        ...
        const confirmed = window.confirm(
            `¿Estás seguro de eliminar el Inventario "${item.name}"?\n\nEsta acción no se puede deshacer.`
        );

        if (confirmed) {
            try {
                setError('');
                await inventoryService.deactivate(item.id);
    ...
    const handleFormSuccess = async (savedItem, isEdit = false) => {
        ...
        const message = isEdit ? 'Inventario actualizado exitosamente' : 'Inventario creado exitosamente';
    ...
    const getBookName = (inventory) => {
        return inventory.book_description || 'Tipo no encontrado';
    };
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center min-h-screen">
                <div className="text-lg text-gray-600">Cargando Inventarios...</div>
            </div>
        );
    }
