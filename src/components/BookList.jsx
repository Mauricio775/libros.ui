...
const BookList = () => {
    ...
    const loadBook = async () => {
        ...
        try {
            ...
            console.error('Error al cargar tipo de libro:', error);
            setError(error.message || 'Error los libros');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (item) => {
        ...
        const confirmed = window.confirm(
            `¿Estás seguro de ${action} el libro "${item.description}"?\n\n${consequence}\n\n¿Deseas continuar?`
        );

        if (confirmed) {
            try {
                setError('');
                await bookService.deactivate(item.id);
                await loadBook();
                const message = hasProducts 
                    ? 'Libro desactivado exitosamente'
                    : 'Libro eliminado exitosamente';
                setSuccessMessage(message);
            } catch (error) {
                console.error('Error al procesar:', error);
                setError(error.message || `Error al ${action} libro`);
            }
        }
    };
    ...
    const handleFormSuccess = async (savedItem, isEdit = false) => {
        ...
        const message = isEdit ? 'Libro actualizado exitosamente' : 'Libro creado exitosamente';
        setSuccessMessage(message);
        setShowForm(false);
        setEditingItem(null);
        setError('');
